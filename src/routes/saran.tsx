...
const JENIS = ["Kata Kerja", "Kata Benda", "Kata Sifat", "Kata Bilangan", "Kata Tugas", "Lainnya"] as const;
    ...
    <Layout>
      ...
      <form onSubmit={submit} className="mx-auto max-w-3xl space-y-4 px-4 py-10">
        <div className="rounded-xl border border-border bg-card p-6">
          <label className="text-sm font-medium" htmlFor="jenisSaran">Jenis Saran</label>
          <select id="jenisSaran" value={form.jenisSaran}
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-sm font-medium" htmlFor="nama">Nama (opsional)</label>
              <input id="nama" maxLength={120} value={form.nama}
                onChange={(e) => setForm({ ...form, nama: e.target.value })}
                className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="text-sm font-medium" htmlFor="email">Email (opsional)</label>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-sm font-medium" htmlFor="katind">Kata Indonesia</label>
              <input id="katind" maxLength={120} value={form.katind}
                onChange={(e) => setForm({ ...form, katind: e.target.value })}
                className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="text-sm font-medium" htmlFor="katNias">Kata Nias (Li Niha)</label>
              <input id="katNias" maxLength={120} value={form.katNias}
          ...
          <div className="mt-4">
            <label className="text-sm font-medium" htmlFor="jenisKata">Jenis Kata</label>
            <select id="jenisKata" value={form.jenisKata}
          ...
          <div className="mt-4">
            <label className="text-sm font-medium" htmlFor="contoh">Contoh Kalimat (opsional)</label>
            <textarea id="contoh" rows={3} maxLength={1000} value={form.contoh}
              onChange={(e) => setForm({ ...form, contoh: e.target.value })}
              className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
          </div>

          <div className="mt-4">
            <label className="text-sm font-medium" htmlFor="catatan">Catatan Tambahan</label>
            <textarea id="catatan" rows={4} maxLength={2000} value={form.catatan}
              onChange={(e) => setForm({ ...form, catatan: e.target.value })}
              className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              placeholder="Mis: kata ini umum dipakai di Nias Selatan, ejaan alternatif, dialek, dsb." />
          </div>

          <button type="submit" className="mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90">
