...
export const Route = createFileRoute("/kata/$slug")({
  loader: async ({ params, context }) => {
    ...
    const related = entries
      .filter((e) => e.id !== entry.id && (e.indo[0]?.toLowerCase() === entry.indo[0]?.toLowerCase() || e.jenis === entry.jenis))
      .slice(0, 8);
  ...
  head: ({ params, loaderData }) => {
      ...
      mainEntity: [
        {
          ...
          acceptedAnswer: { "@type": "Answer", text: `Dalam Bahasa Nias, "${entry.indo}" berarti "${entry.nias}".` },
        },
        {
          "@type": "Question",
          name: `Apa terjemahan "${entry.nias}" ke Bahasa Indonesia?`,
          acceptedAnswer: { "@type": "Answer", text: `"${entry.nias}" diterjemahkan menjadi "${entry.indo}" dalam Bahasa Indonesia.` },
        },
        ...(jenis ? [{
          "@type": "Question",
    ...
    return {
      meta: [
        ...
        { name: "keywords", content: `${entry.indo} bahasa nias, arti ${entry.indo}, terjemahan ${entry.indo}, ${entry.nias} indonesia, kamus nias` },
...
function WordPage() {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try { await navigator.share({ title: entry.indo, text: `${entry.indo} = ${entry.nias}`, url: window.location.href }); } catch {}
    } else { handleCopy(); }
  };
      ...
      <article className="mx-auto max-w-3xl px-4 py-6">
        ...
        <header className="mt-4 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button onClick={handleCopy} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold hover:bg-muted">
              <Copy className="h-3.5 w-3.5" /> {copied ? t.copied : t.copy}
            </button>
            <button onClick={handleSpeak} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold hover:bg-muted">
              <Volume2 className="h-3.5 w-3.5" /> {t.listen}
            </button>
            <button onClick={handleShare} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold hover:bg-muted">
