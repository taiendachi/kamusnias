  ...
  head: () => ({
    ...
    scripts: [{
      ...
      children: JSON.stringify({
        ...
        mainEntity: { "@type": "Organization", name: SITE.organization, email: SITE.contactEmail },
      }),
    }],
  }),
      ...
      <div className="mx-auto grid max-w-3xl gap-6 px-4 py-10 md:grid-cols-[1fr_280px]">
        <form onSubmit={submit} className="space-y-4 rounded-xl border border-border bg-card p-6">
          <div>
            ...
            <input id="nama" required maxLength={120} value={form.nama}
          ...
          <div>
            <label className="text-sm font-medium" htmlFor="subjek">Subjek</label>
            <input id="subjek" maxLength={140} value={form.subjek}
              onChange={(e) => setForm({ ...form, subjek: e.target.value })}
              className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div>
            <label className="text-sm font-medium" htmlFor="pesan">Pesan</label>
            <textarea id="pesan" required rows={6} maxLength={4000} value={form.pesan}
              onChange={(e) => setForm({ ...form, pesan: e.target.value })}
              className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <button type="submit" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90">
            <Send className="h-4 w-4" /> Kirim Pesan
          </button>
          <p className="text-xs text-muted-foreground">
            Tombol di atas akan membuka aplikasi email Anda dan mengisi pesan otomatis ke {SITE.contactEmail}.
          </p>
        </form>
        ...
        <aside className="rounded-xl border border-border bg-card p-6 text-sm">
          ...
          <a href={`mailto:${SITE.contactEmail}`} className="mt-1 block text-primary hover:underline">
            {SITE.contactEmail}
          </a>
          <p className="mt-4 text-muted-foreground">
            Untuk usulan kosakata baru atau koreksi terjemahan, gunakan
            <a href="/saran" className="ml-1 text-primary hover:underline">Form Saran</a>.
          </p>
        </aside>
      </div>
    </Layout>
