...
function renderInline(text: string): ReactNode {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (tok.startsWith("**")) {
      ...
      nodes.push(<code key={i} className="rounded bg-muted px-1 py-0.5 font-mono text-[0.9em]">{tok.slice(1, -1)}</code>);
    } else {
...
export function BlogContent({ content }: { content: string }) {
  ...
  while (i < lines.length) {
    ...
    if (h) {
      ...
      if (level === 2) {
        blocks.push(
          <h2 key={key++} className="mt-10 border-b border-border pb-2 font-serif text-2xl font-bold text-foreground md:text-[1.7rem]">
            {renderInline(text)}
          </h2>,
        );
      } else if (level === 3) {
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-muted/70">
                {header.map((c, j) => (
                  <th key={j} className="border-b border-border px-3 py-2 text-left font-semibold">{renderInline(c)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {body.map((row, r) => (
                <tr key={r} className="even:bg-muted/30">
                  {row.map((c, j) => (
                    <td key={j} className="border-b border-border px-3 py-2 align-top last:border-b-0">{renderInline(c)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
  ...
  return <div className="mt-6 max-w-none text-[0.95rem] text-foreground/90 md:text-base">{blocks}</div>;
}
